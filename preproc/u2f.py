...
def main():
    ...
    with open(serpentfilename, "w") as srpf:
        ...
        for (c, s) in zip(classnames, signature_set):
            ...
            chans_list = "" if s.output.fixed else "chans"
            for p in s.params:
                if p.fixed:
                    achans = p.name + ".chans"
                    if p.chans == 1:
    ...
    ### create class.py
    pythonfilename = classnamelc + ".py"
    print("**** Generating", pythonfilename)
    with open(pythonfilename, "w") as pyf:
        print("#", pythonfilename, "- constructor implementation", file=pyf)
        print("#\n# (machine generated by u2f.py)\n", file=pyf)

        print("from pyarco.arco_ugens import *\n", file=pyf)
        ...
        for (c, s) in zip(classnames, signature_set):
            ...
            for p in s.params:
                ...
                # a block-rate input.
                #
                # In Python, we use 'c' for the rate of Const even
                # though it looks like a 'b' rate to any consumer.
                # We also use 'f' (for "fixed" or "float") for parameters
                # that are "fixed" or "set" at instantiation time and
                # maybe are updateable using methods. The 'f' type is
                ...
                # an a-rate to a b-rate only input, but we want to
                # allow connecting a c-rate (Const) to a b-rate input,
                # we have to convert all the types here for Python:
                # "ab" becomes "abc", "b" becomes "bc". "c" becomes "f".
                # There should not be an "ac", "abc", or "bc" in the
                # .ugen file.
                # 
            ...
            for p in s.params:
                if p.fixed:
                    ...
                    chans_list += ", " + p.name
            if s.output.fixed:
                print(chans_list, file=pyf)
            else:  # check each input for required specified channel count or 1
                print("    chans = max_chans(" + chans_list + ")", file=pyf)
            omit_chans = "True" if s.output.fixed else "False"
            chans = s.output.chans if s.output.fixed else "chans"

            rate = "A_RATE" if s.output.abtype == 'a' else "B_RATE"
            
            ugen = f'    return Ugen(new_ugen_id(), "{c}", {chans}, ' + \
                   f'{rate}, "{typestring}", False, {omit_chans}{srpparams})'
            ugen = line_wrap(ugen, 9)
            print(ugen, file=pyf)
            print("", file=pyf)
