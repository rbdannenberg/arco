...
class Sine : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "sine_bb.dsp");
		m->declare("interoplate", "amp");
	...
	static void classInit(int sample_rate) {
		...
		sig0->instanceInitSineSIG0(sample_rate);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow1 = (fConst0 * float(fEntry1));
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			fRec1[0] = (fSlow1 + (fRec1[1] - std::floor((fSlow1 + fRec1[1]))));
			output0[i0] = FAUSTFLOAT((fSlow0 * ftbl0SineSIG0[int((65536.0f * fRec1[0]))]));
			fRec1[1] = fRec1[0];
		}
	}
