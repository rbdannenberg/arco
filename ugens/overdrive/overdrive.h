...
public:
    ...
    void chan_abbb_a(Overdrive_state *state) {
        ...
        float fSlow0 = 1.0f / std::tan(fConst1 * (4.15e+03f * float(tone_samps[0]) + 3.5e+02f));
        float fSlow1 = 1.0f - fSlow0;
        float fSlow2 = std::pow(1e+01f, 2.0f * float(gain_samps[0]));
        float fSlow3 = 1.0f / (fSlow0 + 1.0f);
        float fSlow4 = std::pow(1e+01f, 2.0f * (float(volume_samps[0]) + -1.0f));
        for (int i0 = 0; i0 < BL; i0 = i0 + 1) {
            float fTemp0 = float(input0[i0]) + float(input1[i0]);
            state->fVec0[0] = fTemp0;
            state->fRec1[0] = -(fConst4 * (fConst3 * state->fRec1[1] + fConst2 * (state->fVec0[1] - fTemp0)));
            float fTemp1 = std::max<float>(-1.0f, std::min<float>(1.0f, fSlow2 * state->fRec1[0]));
            float fTemp2 = fTemp1 * (1.0f - 0.33333334f * Overdrive_faustpower2_f(fTemp1));
            state->fVec1[0] = fTemp2;
            state->fRec0[0] = fSlow3 * (fTemp2 + state->fVec1[1] - fSlow1 * state->fRec0[1]);
            float fTemp3 = fSlow4 * state->fRec0[0];
