// fixphase.cpp - based on Dannenberg and Thompson, CMJ, implements
//     unit generators in the style of Arco
// WITH FIXED POINT PHASE IN TABLEOSC
//
// Roger B. Dannenberg
// May 2025

const int LOG2_BL = 5;
#define RESULTS "data/fixphase.txt"
...
public:
    ...
    void create_table_at(int i, int tlen) {
        ...
        if (n <= i) {
            ...
            for (; n < i + 1; n++) {
                wavetables[i].wavetable.init(0);
            }
        }
        wavetables[i].wavetable.set_size(tlen + 2, false);
        wavetables[i].log2len = ilog2(tlen);
    }
    ...
    void create_table(int i, int tlen, int slen, float *spec, bool hasphase) {
        ...
        Wavetable &table = wavetables[i].wavetable;
        table.zero();
...
public:
    struct Tableosc_state {
        int64_t phase;  // from 0 to 1 (1 represents 2PI or 360 degrees)
        Sample prev_amp;
    };
    int which_table;
    ...
    void chan_aa_a(Tableosc_state *state, Sample *table, int log2len) {
        uint64_t phase = state->phase;
        int64_t fixed_2pi = (1 << log2len);
        float fixed_2pi_inv = 1.0f / (float) fixed_2pi;
        for (int i = 0; i < BL; i++) {
            int64_t ix = phase >> log2len;
            int64_t frac = phase - (ix << log2len);
            *out_samps++ = (table[ix] * (fixed_2pi - frac) + 
                    table[ix + 1] * frac) * fixed_2pi_inv * amp_samps[i];
            phase += freq_samps[i] * AP * fixed_2pi;
            phase &= (fixed_2pi - 1);
        }
        state->phase = phase;
    }
    ...
    void chan_ba_a(Tableosc_state *state, Sample *table, int log2len) {
        uint64_t phase = state->phase;
        int64_t fixed_2pi = (1 << log2len);
        float fixed_2pi_inv = 1.0f / (float) fixed_2pi;
        uint64_t phase_incr = *freq_samps * AP * fixed_2pi;
        for (int i = 0; i < BL; i++) {
            int64_t ix = phase >> log2len;
            int64_t frac = phase - (ix << log2len);
            *out_samps++ = (table[ix] * (fixed_2pi - frac) + 
    ...
    void chan_ab_a(Tableosc_state *state, Sample *table, int log2len) {
        uint64_t phase = state->phase;
        int64_t fixed_2pi = (1 << log2len);
        float fixed_2pi_inv = 1.0f / (float) fixed_2pi;
        ...
        for (int i = 0; i < BL; i++) {
            ...
            *out_samps++ = (table[ix] * (fixed_2pi - frac) + 
                    table[ix + 1] * frac) * fixed_2pi_inv * amp_sig_fast;
            phase += freq_samps[i] * AP * fixed_2pi;
            phase &= (fixed_2pi - 1);
        }
        state->phase = phase;
    ...
    void chan_bb_a(Tableosc_state *state, Sample *table, int log2len) {
        uint64_t phase = state->phase;
        int64_t fixed_2pi = (1 << log2len);
        float fixed_2pi_inv = 1.0f / (float) fixed_2pi;
        uint64_t phase_incr = *freq_samps * AP * fixed_2pi;
    ...
    void real_run() {
        ...
        Wtable *table = get_table(which_table);
        if (!table) {
            return;
        }
        int tlen = table->wavetable.size() - 2;
        if (tlen < 2) {
            return;
        }
        int log2len = ilog2(tlen);
        for (int i = 0; i < chans; i++) {
            (this->*run_channel)(state, &(table->wavetable[0]), log2len);
            state++;
