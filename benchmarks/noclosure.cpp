...
public:
    ...
    void real_run() {
        freq_samps = freq->out_samps; // update input
        amp_samps = amp->out_samps; // update input
...
#define APPLY_X1_X2_GAIN \
    ...
    void real_run() {
        x1_samps = x1->out_samps;  // update input
        x2_samps = x2->out_samps; // update input
        b_samps = b->out_samps;  // update input
...
public:
    ...
    void real_run() {
        ...
        // left now represents the target value
        left = state->left;  // replace it with the current left value
        ...
        // right now represents the target value
        right = state->right;  // replace it with the current right value
        ...
        state->left = left;  // save final left and right for next block
        state->right = right;
...
public:
    ...
    void real_run() {
        ...
        while (i < inputs.size()) {
            ...
            Sample_ptr input_ptr = input->out_samps;
...
public:
    ...
    void real_run() {
        x1_samps = x1->out_samps;
        x2_samps = x2->out_samps;
...
int main()
{
    ...
    while (block_count < duration_blocks) {
        block_count += 1;
        Sample_ptr save_out_samps;

        save_out_samps = vibamp_pwlb->out_samps;
        vibamp_pwlb->current_block = block_count;
        vibamp_pwlb->real_run();
        vibamp_pwlb->out_samps = save_out_samps;

        save_out_samps = vibfreq_pwlb->out_samps;
        vibfreq_pwlb->current_block = block_count;
        vibfreq_pwlb->real_run();
        vibfreq_pwlb->out_samps = save_out_samps;

        save_out_samps = vibosc_sineb->out_samps;
        vibosc_sineb->current_block = block_count;
        vibosc_sineb->real_run();
        vibosc_sineb->out_samps = save_out_samps;

        save_out_samps = freq_pwlb->out_samps;
        freq_pwlb->current_block = block_count;
        freq_pwlb->real_run();
        freq_pwlb->out_samps = save_out_samps;

        save_out_samps = modfreq_mathb->out_samps;
        modfreq_mathb->current_block = block_count;
        modfreq_mathb->real_run();
        modfreq_mathb->out_samps = save_out_samps;

        save_out_samps = osc1_tableosc->out_samps;
        osc1_tableosc->current_block = block_count;
        osc1_tableosc->real_run();
        osc1_tableosc->out_samps = save_out_samps;

        save_out_samps = osc2_tableosc->out_samps;
        osc2_tableosc->current_block = block_count;
        osc2_tableosc->real_run();
        osc2_tableosc->out_samps = save_out_samps;

        save_out_samps = xfade_pwlb->out_samps;
        xfade_pwlb->current_block = block_count;
        xfade_pwlb->real_run();
        xfade_pwlb->out_samps = save_out_samps;

        save_out_samps = blend->out_samps;
        blend->current_block = block_count;
        blend->real_run();
        blend->out_samps = save_out_samps;

        save_out_samps = pan_pwlb->out_samps;
        pan_pwlb->current_block = block_count;
        pan_pwlb->real_run();
        pan_pwlb->out_samps = save_out_samps;

        save_out_samps =pan->out_samps;
        pan->current_block = block_count;
        pan->real_run();
        pan->out_samps = save_out_samps;

        save_out_samps =sum->out_samps;
        sum->current_block = block_count;
        sum->real_run();
        sum->out_samps = save_out_samps;
...
#define RESULTS "data/noclosure.txt"
#include "report.cpp"
}
