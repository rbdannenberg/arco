// fastchannels.cpp - based on Dannenberg and Thompson, CMJ, implements
//     unit generators in the style of Arco.
//     Try to access state vectors without bounds checking
//
// Roger B. Dannenberg
// Jan 2025
...
public:
    ...
    Tableosc(int id, int nchans, Ugen_ptr freq_, Ugen_ptr amp_, float phase) :
        ...
        states_array = states.get_array();
        for (int i = 0; i < chans; i++) {
            states_array[i].phase = fmodf(phase / 360.0f, 1.0f);
        }
        init_freq(freq_);
        init_amp(amp_);
        update_run_channel();
    ...
    void real_run() {
        ...
        Tableosc_state *state = states_array;
...
public:
    ...
    void initialize_channel_states() {
        for (int i = 0; i < chans; i++) {
            for (int l2 = 0; l2 < 2; l2 = l2 + 1) {
                states_array[i].iVec1[l2] = 0;
            }
            for (int l3 = 0; l3 < 2; l3 = l3 + 1) {
                states_array[i].fRec1[l3] = 0.0f;
            }
        }
    }
    ...
    void real_run() {
        RUN(freq_samps, freq, current_block);  // update input
        RUN(amp_samps, amp, current_block);  // update input
        Sineb_state *state = states_array;
        for (int i = 0; i < chans; i++) {
...
#define APPLY_X1_X2_GAIN \
    ...
    void real_run() {
        ...
        Blend_state *state = states_array;
        for (int i = 0; i < chans; i++) {
...
public:
    ...
    void real_run() {
        ...
        Stpan_state *state = states_array;
    ...
    void real_run() {
        ...
        for (int i = 0; i < chans; i++) {
            switch (op) {
              ...
              case MATH_OP_SH: {
                    Mathb_state *state = states_array + i;
                    Sample h = state->hold;
                    Sample p = state->prev;
...
#define RESULTS "data/fastchannels.txt"
#include "report.cpp"
}
