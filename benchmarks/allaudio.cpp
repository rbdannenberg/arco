// allaudio.cpp - based on Dannenberg and Thompson, CMJ, implements
...
Compilation options: -lang cpp -os -light -ct 1 -cn Sine -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
public:
    ...
    void chan_ba_a(Sine_state *state) {
        ...
        for (int i0 = 0; i0 < BL; i0 = i0 + 1) {
            ...
            output0[i0] = FAUSTFLOAT(float(input0[i0]) * ftbl0SineSIG0[std::max<int>(0, std::min<int>(int(SINESIZE * state->fRec1[0]), (SINESIZE - 1)))]);
    ...
    void chan_bb_a(Sine_state *state) {
        ...
        for (int i0 = 0; i0 < BL; i0 = i0 + 1) {
            ...
            state->fRec1[0] = fTemp0 - std::floor(fTemp0);
            output0[i0] = FAUSTFLOAT(fSlow0_fast * ftbl0SineSIG0[std::max<int>(0, std::min<int>(int(SINESIZE * state->fRec1[0]), (SINESIZE - 1)))]);
            state->iVec1[1] = state->iVec1[0];
            state->fRec1[1] = state->fRec1[0];
        }
    }
    ...
    void chan_aa_a(Sine_state *state) {
        ...
        for (int i0 = 0; i0 < BL; i0 = i0 + 1) {
            ...
            state->fRec1[0] = fTemp0 - std::floor(fTemp0);
            output0[i0] = FAUSTFLOAT(ftbl0SineSIG0[std::max<int>(0, std::min<int>(int(SINESIZE * state->fRec1[0]), (SINESIZE - 1)))] * float(input1[i0]));
            state->iVec1[1] = state->iVec1[0];
            state->fRec1[1] = state->fRec1[0];
        }
    }
...
public:
    ...
    void update_run_channel() {
        if (mode == BLEND_POWER) {
            ...
            run_channel = &Blend::linear_aaa_a;
        }
    }
...
#define APPLY_X1_X2_GAIN \
    ...
    void linear_aaa_a(Blend_state *state) {
        for (int i = 0; i < BL; i++) {
            *out_samps++ = gain * (x1_samps[i] * (1.0f - b_samps[i]) +
                                   x2_samps[i] * b_samps[i]);
        }
    }
...
Pwl_ptr vibamp_pwl;
Pwl_ptr vibfreq_pwl;
Ugen_ptr vibosc_sine;
Pwl_ptr freq_pwl;
Ugen_ptr modfreq_math;
...
void build_graph()
{
    vibamp_pwl = new Pwl(10);
    vibamp_pwl->points.clear();
    vibamp_pwl->point(AR * 2);
    vibamp_pwl->point(10.0);
    vibamp_pwl->start();

    vibfreq_pwl = new Pwl(11);
    vibfreq_pwl->points.clear();
    vibfreq_pwl->point(AR * 1);
    vibfreq_pwl->point(6.0);
    vibfreq_pwl->start();

    vibosc_sine = new Sine(12, 1, vibfreq_pwl, vibamp_pwl);

    freq_pwl = new Pwl(13);;
    freq_pwl->points.clear();
    freq_pwl->set(440.0);
    freq_pwl->point(AR * SIMSECS);
    freq_pwl->point(440.0);
    freq_pwl->start();

    modfreq_math = new Math(14, 1, MATH_OP_ADD, freq_pwl, vibosc_sine);

    gain_pwl = new Pwl(20);
    gain_pwl->points.clear();
    gain_pwl->point(AR * 1);
    gain_pwl->point(1.0);
    gain_pwl->start();
    ...
    osc1_tableosc = new Tableosc(16, 1, modfreq_math, gain_pwl, 0.0);
    osc1_tableosc->create_table(0, 2048, 4, spectrum1, false);

    osc2_tableosc = new Tableosc(17, 1, modfreq_math, gain_pwl, 0.0);
    osc2_tableosc->create_table(0, 2048, 4, spectrum2, false);

    xfade_pwl = new Pwl(18);
    xfade_pwl->points.clear();
    xfade_pwl->point(AR * SIMSECS);
    xfade_pwl->point(1.0);
    xfade_pwl->start();

    blend = new Blend(19, 1, osc1_tableosc, osc2_tableosc, xfade_pwl, 0.0,
                  BLEND_LINEAR);

    pan_pwl = new Pwl(21);
    pan_pwl->points.clear();
    pan_pwl->set(0.4);
    pan_pwl->point(AR * SIMSECS);
    pan_pwl->point(0.6);
    pan_pwl->start();

    pan = new Stpan(22, 2, blend, pan_pwl);
